...
const Notifikasi = () => {
  ...
  const fetchUsers = async () => {
    try {
      const response = await fetch("http://192.168.1.9:3003/users");
  ...
  const fetchMessages = async () => {
    try {
      const response = await fetch("http://192.168.1.9:3003/pesan");
      const data = await response.json();
      setMessages(data);
    } catch (error) {
      console.error(error);
  ...
  const updateMessageStatus = async (messageIds) => {
    try {
      for (const messageId of messageIds) {
        await fetch(`http://192.168.1.9:3003/pesan/${messageId}`, {
          method: "PUT",
        });
      }
    } catch (error) {
